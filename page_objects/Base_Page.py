...
class Base_Page(Borg, unittest.TestCase):
    ...
    def register_driver(self, remote_flag, os_name, os_version, browser, browser_version, remote_project_name, remote_build_name):
        ...
        self.set_screenshot_dir(os_name, os_version, browser, browser_version)
        self.set_log_file()
        self.driver = self.driver_obj.get_web_driver(
            remote_flag, os_name, os_version, browser, browser_version, remote_project_name, remote_build_name)
    ...
    def set_screenshot_dir(self, os_name, os_version, browser, browser_version):
        ...
        try:
            self.screenshot_dir = self.get_screenshot_dir(
                os_name, os_version, browser, browser_version, overwrite_flag=True)
    ...
    def get_screenshot_dir(self, os_name, os_version, browser, browser_version, overwrite_flag=False):
        ...
        if isinstance(os_name, list):
            ...
            windows_browser_combination = os_name.lower() + '_' + str(os_version).lower() + \
                '_' + browser.lower() + '_' + str(browser_version)
        self.testname = self.get_calling_module()
        self.testname = self.testname.replace('<', '')
        self.testname = self.testname.replace('>', '')
        self.testname = self.testname + \
            '[' + str(windows_browser_combination) + ']'
    ...
    def set_log_file(self):
        ...
        self.log_obj = Base_Logging(
            log_file_name=self.log_name, level=logging.DEBUG)
    ...
    def _get_locator(key):
        ...
        value = None
        try:
            path_conf_file = os.path.abspath(os.path.join(
                os.path.dirname(__file__), '..', 'conf', 'locators.conf'))
        ...
        try:
            ...
            if msg_flag == True:
                ...
                self.write(
                    "Check your locator-'%s,%s' in the conf/locators.conf file" % (locator[0], locator[1]))
            self.exceptions.append(
                "Unable to locate the element with the xpath -'%s,%s' in the conf/locators.conf file" % (locator[0], locator[1]))
    ...
    def set_text(self, locator, value, clear_flag=True):
        ...
        try:
            ...
            self.write("Check your locator-'%s,%s' in the conf/locators.conf file" %
                       (locator[0], locator[1]))
        ...
        if text_field is not None:
            try:
                ...
                self.write('Could not write to text field: %s' %
                           locator, 'debug')
                self.write(str(e), 'debug')
                self.exceptions.append(
                    "Could not write to text field- '%s' in the conf/locators.conf file" % locator)
    ...
    def get_dom_text(self, dom_element):
        ...
        try:
            ...
            self.write(e)
            self.exceptions.append(
                "Error when getting text from the DOM element-'%s' in the conf/locators.conf file" % locator)
    ...
    def select_checkbox(self, locator):
        ...
        try:
            ...
            self.write(e)
            self.exceptions.append(
                "Error when selecting checkbox-'%s' in the conf/locators.conf file" % locator)
    ...
    def deselect_checkbox(self, locator):
        ...
        try:
            ...
            self.write(e)
            self.exceptions.append(
                "Error when deselecting checkbox-'%s' in the conf/locators.conf file" % locator)
    ...
    def toggle_checkbox(self, locator):
        ...
        try:
            ...
            self.write(e)
            self.exceptions.append(
                "Error when toggling checkbox-'%s' in the conf/locators.conf file" % locator)
    ...
    def select_dropdown_option(self, locator, option_text):
        ...
        try:
            ...
            self.write(e)
            self.exceptions.append(
                "Error when selecting option from the drop-down '%s' " % locator)
    ...
    def write(self, msg, level='info'):
        ...
        if self.browserstack_flag is True:
            if self.browserstack_msg not in msg:
                # Remove the redundant BrowserStack message
                self.msg_list.pop(-1)
    ...
    def report_to_testrail(self, case_id, test_run_id, result_flag, msg=''):
        ...
        if self.testrail_flag is True:
            ...
            if self.browserstack_flag is True:
                ...
                msg += '\n\n' + \
                    '[' + 'Watch Replay On BrowserStack' + \
                    '](' + self.session_url+')'
            self.tr_obj.update_testrail(
                case_id, test_run_id, result_flag, msg=msg)
        self.image_url_list = []
        self.msg_list = []
    ...
    def add_tesults_case(self, name, desc, suite, result_flag, msg='', files=[], params={}, custom={}):
        ...
        if self.tesults_flag is True:
            ...
            self.images = []
            caseObj = {'name': name, 'suite': suite, 'desc': desc, 'result': result,
                       'reason': failReason, 'files': files, 'params': params}
