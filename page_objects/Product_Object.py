...
class Product_Object():
    ...
    def convert_str_to_int(self, string, default=100000, expect_fail=False):
        ...
        try:
            ...
            if not expect_fail:
                self.write(
                    "Unable to convert the string %s into a number" % string)
            integer = default
    ...
    def get_minimum_priced_product(self, filter_condition):
        ...
        for product in all_products:
            if filter_condition.lower() in product.name.lower():
                # min price is assigned with max price .so logic should be less than , to work.
        ...
        self.conditional_write(result_flag,
                               positive="Min price for product with '%s' is %s with price %d" % (
                                   filter_condition, min_name, min_price),
    ...
    def click_add_product_button(self, product_name):
        ...
        result_flag = self.click_element(
            self.ADD_PRODUCT_BUTTON % product_name)
        self.conditional_write(result_flag,
    ...
    def add_product(self, product_name):
        ...
        result_flag &= True if after_cart_quantity - \
            before_cart_quantity == 1 else False
    ...
    def verify_cart_page(self):
        ...
        if result_flag:
            # Page redirection was to main and main was not having the reference to cart_page
