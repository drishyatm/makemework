...
class Main_Page(Base_Page):
    ...
    def get_temperature(self):
        ...
        if temperature is not None:
            self.write("The temperature parsed is: %s" %
                       temperature, level="debug")
            # Strip away the degree centigrade
            # changed the selector from 1 to 0
            temperature = temperature.split()[0]

            try:
                temperature = int(temperature)
            except Exception as e:
                self.write("Error type casting temperature to int",
                           level="error")
    ...
    def click_buy_button(self, product_type):
        ...
        if product_type in ['sunscreens', 'moisturizers']:
            ...
            result_flag &= self.smart_wait(
                5, self.PAGE_HEADING % product_type.title())
            if result_flag:
