...
class Cart_Page(Base_Page):
    ...
    def process_item(self, item):
        ...
        try:
            ...
            self.write("Unable to convert the string %s into a number" %
                       item[self.COL_PRICE])
    ...
    def get_cart_items(self):
        ...
        for index, row in enumerate(row_elements):
            column_elements = self.get_elements(
                self.CART_ROW_COLUMN % (index+1))
            item = []
    ...
    def verify_extra_items(self, expected_cart, actual_cart):
        ...
        for item in actual_cart:
            ...
            self.conditional_write(found_flag,
                                   positive="Found the expected item '%s' in the cart" % item[
                                       self.COL_NAME],
    ...
    def get_total_price(self):
        ...
        try:
            ...
            self.write(
                "Could not convert '%s' (cart total price) into an integer" % actual_price)
    ...
    def verify_cart_total(self, expected_cart):
        ...
        expected_total = 0
        for product in expected_cart:
            expected_total += product.price  # added + to calculate the price correctly
